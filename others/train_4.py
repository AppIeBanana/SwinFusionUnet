...
# from Utils.generalized_dice_loss import generalized_dice_loss
...
def train_model(model, learning_rate, batch_size, loss_rate, epochs):
    ...
    args = get_args()
    train_loader = DataLoader(dataset=MyDataSet('train', args.img_size, args.num_classes), batch_size=batch_size,
                              shuffle=True, num_workers=2,
                              drop_last=True)
    val_loader = DataLoader(dataset=MyDataSet('val', args.img_size, args.num_classes), batch_size=batch_size,
    ...
    optimizer = optim.Adam(model.parameters(), lr=learning_rate, betas=(0.9, 0.999), eps=1e-8, weight_decay=1e-8,
                           amsgrad=False)
    ...
    best_loss = float('inf')
    best_dice = float('inf')
    log_dir_new = '{}_bt{}_img{}_cls{}'.format(datetime.date.today(), batch_size, args.img_size, args.num_classes)
    log_dir = os.path.join(log_dir_root, log_dir_new)
    ...
    with open('../log.md', 'a') as f:
        f.write(
            ...
            f'n_chans_x:{args.in_chans_x},n_chans_y:{args.in_chans_y}, lr: {learning_rate}, eps: {args.eps}, weight_decay: {args.weight_decay}, batch_size:{batch_size},dice_ce_rate{loss_rate}\n')
    print('Start Train')
    for epoch in range(1, epochs + 1):
        torch.cuda.empty_cache()
        model.train()
        train_loss, train_dice, test_loss, test_dice = 0, 0, 0, 0
        for (x, y, label) in tqdm(train_loader, desc=f'Epoch {epoch}/{epochs}'):
            x, y, label = x.to(device, torch.float32), y.to(device, torch.float32), label.to(device, torch.long)
            with torch.cuda.amp.autocast(enabled=args.amp):
                ...
                single_train_loss = (1 - loss_rate) * criterion(out, label) + loss_rate * dice_loss(
                    ...
                    multiclass=True)
                # single_train_loss = (1 - loss_rate) * criterion(out, label) + loss_rate * generalized_dice_loss(
                #     F.softmax(out, dim=1).float(),
                #     F.one_hot(label, args.num_classes).permute(0, 3, 1, 2).float())
        ...
        if epoch % 1 == 0:
            test_dice, test_loss, OA, AA, Kappa, acc0, acc1, acc2, acc3 = multi_dice_score(model,
                                                                                           val_loader,
                                                                                           device,
                                                                                           loss_rate,
            ...
            if test_loss < best_loss:
                ...
                torch.save(model.state_dict(),
                           str(dir_checkpoint / '_{}_SwinFUNet_BestLoss_img{}_class{}_lr{}_bt{}_rate{}_ep{}.pth'.format(
                               datetime.date.today(), args.img_size, args.num_classes, learning_rate, batch_size,
                               loss_rate,
                               epochs)))
        ...
        model.eval()
    new_dir = '{}_bt{}_img{}_cls{}'.format(datetime.date.today(), batch_size, args.img_size, args.num_classes)
