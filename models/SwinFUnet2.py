...
class Fusion_WindowAttention(nn.Module):
    r""" Window based multi-head self attention (W-MSA) module with relative position bias.
    It supports both of shifted and non-shifted window.
...
class Fusion_SwinTransformerBlock(nn.Module):
    r""" Swin Transformer Block.
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.attn_A = Fusion_WindowAttention(
            dim, window_size=to_2tuple(self.window_size), num_heads=num_heads,
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)

        self.attn_B = Fusion_WindowAttention(
            dim, window_size=to_2tuple(self.window_size), num_heads=num_heads,
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)
...
class Fusion_PatchExpand(nn.Module):
    def __init__(self, input_resolution, dim, dim_scale=2, norm_layer=nn.LayerNorm):
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim
...
class Fusion_BasicLayer(nn.Module):
    """ A basic Swin Transformer layer for one stage.
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        # build blocks
        self.blocks = nn.ModuleList([
            Fusion_SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                        num_heads=num_heads, window_size=window_size,
                                        shift_size=0 if (i % 2 == 0) else window_size // 2,
                                        mlp_ratio=mlp_ratio,
                                        qkv_bias=qkv_bias, qk_scale=qk_scale,
    ...
    def forward(self, x, y):
        for blk in self.blocks:
            if self.use_checkpoint:
                x, x_size = checkpoint.checkpoint(blk, x, y)
            else:
                x, x_size = blk(x, y)
        if self.downsample is not None:
            x = self.downsample(x)
...
class RSTB_up(nn.Module):
    ...
    def forward(self, x):
        x_res, x_upsample = self.residual(x)
        return x_res + x, x_upsample
...
class Fusion_BasicLayer_up(nn.Module):
    """ A basic Swin Transformer layer for one stage.
    ...
    def forward(self, x, y):
        for blk in self.blocks:
            if self.use_checkpoint:
                x, _ = checkpoint.checkpoint(blk, x, y)
            else:
                x, _ = blk(x, y)
        if self.upsample is not None:
            x = self.upsample(x)
...
class SwinTransformerFusion(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans_x=100, in_chans_y=3, num_classes=5,
                 ...
                 use_checkpoint=False, final_upsample="expand_first", random_drop_rate=0, **kwargs):
        super().__init__()
        ...
        self.patches_resolution = patches_resolution
        self.res_drop_rate = nn.Dropout(p=random_drop_rate)
        ...
        for i_layer in range(self.num_layers):
            layer_fusion = Fusion_BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                                             input_resolution=(patches_resolution[0] // (2 ** i_layer),
                                                               patches_resolution[1] // (2 ** i_layer)),
                                             depth=depths[i_layer],
                                             num_heads=num_heads[i_layer],
        ...
        for i_layer in range(self.num_layers):
            concat_linear_fusion = nn.Linear(2 * int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                                             int(embed_dim * 2 ** (
                                                     self.num_layers - 1 - i_layer))) if i_layer > 0 else nn.Identity()
            if i_layer == 0:
                layer_up_fusion = Fusion_PatchExpand(
                    input_resolution=(patches_resolution[0] // (2 ** (self.num_layers - 1 - i_layer)),
                                      patches_resolution[1] // (2 ** (self.num_layers - 1 - i_layer))),
                    dim=int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)), dim_scale=2, norm_layer=norm_layer)
            else:
                layer_up_fusion = Fusion_BasicLayer_up(dim=int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                                                       input_resolution=(
                                                       ...
                                                       drop_path=dpr[sum(depths[:(self.num_layers - 1 - i_layer)]):sum(
                                                           depths[:(self.num_layers - 1 - i_layer) + 1])],
                                                       norm_layer=norm_layer,
                                                       upsample=Fusion_PatchExpand if (
    ...
    def forward_features_Fusion(self, x, y):
        ...
        for layer_x, layer_y, layer_fusion in zip(self.layers_x, self.layers_y, self.layers_Fusion):
            ...
            x, x_res = layer_x(x)  # x为未经下采样的Transformer输出,x_res用于残差
            y_downsample.append(y)
            y, y_res = layer_y(y)

            # 下采样
            x, x_size = layer_fusion(x, y)
            x_sizes.append(x_size)
            y_sizes.append(y_res)

            x = x_res + self.res_drop_rate(x)
            y = y_res
    ...
    def forward_features_up_Fusion(self, x, x_downsample, y, y_downsample, x_sizes, y_sizes):
        for layer_up_x, layer_up_y, (inx, layer_up_fusion) in zip(self.layers_up_x, self.layers_up_y,
            ...
            if inx == 0:
                ...
                x = x_expand + self.res_drop_rate(x)
                y = y_expand + self.res_drop_rate(y)
            else:
                ...
                x = x_res_up + self.res_drop_rate(x)
                y = y_res_up
                # concatnate x,y in the channel dimension
                # x = torch.cat([x, y], 2)  # B, L, C
                # x = x.permute(0, 2, 1)  # B,C,L
...
if __name__ == '__main__':
    ...
    model = SwinTransformerFusion(img_size=224, patch_size=4, in_chans_x=8, in_chans_y=8, num_classes=5,
