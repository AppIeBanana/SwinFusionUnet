...
class Channel_Embeddings(nn.Module):
    ...
    def __init__(self, patchsize, img_size, in_channels):
        # 首先，通过调用 super().__init__() 来初始化父类 nn.Module。
        super().__init__()
        # 使用 _pair 函数将 img_size 和 patchsize 转换为二元组。
        # 这是为了确保 img_size 和 patchsize 的形状正确。
        img_size = _pair(img_size)
        patch_size = _pair(patchsize)
        # 接下来，计算 n_patches，即补丁的数量。
        # 它是通过将图像的高度和宽度分别除以补丁的高度和宽度得到的。
        ...
        # 使用 Dropout 创建一个丢弃层，丢弃率为 0.1。这将用于在嵌入向量中应用随机丢弃，以减少过拟合。
        self.dropout = Dropout(0.1)

    def forward(self, x):
        if x is None:
            return None
        # 卷积操作会将输入图像划分为补丁，并为每个补丁提取特征。
        x = self.patch_embeddings(x)  # (B, hidden. n_patches^(1/2), n_patches^(1/2))
        x = x.flatten(2)
        # 通过转置操作将张量的最后两个维度交换，得到形状为 (B, n_patches, hidden) 的张量。这是为了将补丁特征的维度放在最后一个维度上。
        x = x.transpose(-1, -2)  # (B, n_patches, hidden)
        # 将位置嵌入 self.position_embeddings 加到补丁特征上，得到最终的嵌入向量。
        embeddings = x + self.position_embeddings
        embeddings = self.dropout(embeddings)
        return embeddings

class Reconstruct(nn.Module):  # 重建图像
...
class Attention_org(nn.Module):  # 多头注意力机制，可以对多个输入嵌入进行注意力计算，并生成相应的输出。
...
class Mlp(nn.Module):  # 这段代码实现了一个简单的MLP模型，它由两个线性层和一个激活函数组成。
    # 通过前向传播过程，输入数据经过线性变换和非线性激活后，得到最终的输出。
...
class Block_ViT(nn.Module):  # vision—transformer 包含了通道注意力机制和多层感知机层
    def __init__(self, vis, channel_num):
