...
from UCTransNet.CTrans import ChannelTransformer  # train时候用的

def get_activation(activation_type):    #用于指定激活函数的类型
    activation_type = activation_type.lower()
    if hasattr(nn, activation_type):
        return getattr(nn, activation_type)()  # 如果activation_type是torch.nn模块中的有效属性，函数将使用getattr()函数获取该属性对应的激活函数，并返回该激活函数的实例。getattr()函数可以根据属性名称动态获取对象的属性。
    else:
        return nn.ReLU()
...
class UpBlock_attention(nn.Module):
    def __init__(self, in_channels, out_channels, nb_Conv, activation='ReLU'):
        ...
        # self.SELayer = SELayer(channel=in_channels*2)
...
class SELayer(nn.Module):
    ...
    def forward(self, x):
        ...
        y = y.expand_as(x)  # Expand y to have the same shape as x
        return x + x * y  # Add residual connection
...
class UCTransNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # SE4和SE5留一个即可，看是经过CTrans还是不经过，之前跑的是经过的。
...
if __name__ == '__main__':
    ...
    model = UCTransNet(n_channels=16, n_classes=7)
    y= model(x)
    print(y.shape)
    # for feature in y2:
