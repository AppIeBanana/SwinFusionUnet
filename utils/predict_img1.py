...
def show_gray_img(y, y_, windows_size, n_classes, dice):
    idxs = [0, 1, 2, 3, 4]
    # if n_classes == 2:
    #     # 这五张图像是先输出一下所有（150张）图片，然后挑选一些质量好的 用数组存起来，（later的操作）然后读取数组展示质量比较好的。
    #     idxs = [2, 3, 5, 12, 16]  # 2分类
    # if n_classes == 7:    #六分类，7=6+1（1是背景）
    #     idxs = [0, 3, 7, 11, 19]
    plt.figure()
    for idx in range(len(idxs)):
        ...
        # cmap“terrain”看起来很棒但是颜色缩放不是基于零
        # (即,如果比例为0-> 5000m,则0-> 1000m范围可能是蓝色阴影,你可以认为是海底-水平)
        plt.imshow(img2, cmap=plt.cm.terrain)
    # 只存一次图
    plt.savefig("./result/result_rgb_{}分类_dice{}.jpg".format(n_classes, dice))
    plt.show()
...
if __name__ == "__main__":
    batch_size = 2
    n_classes = 2
    test_loader = DataLoader(dataset=dataset.genTestSet(), batch_size=batch_size, shuffle=True)
