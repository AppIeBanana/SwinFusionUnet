...
def dice_coeff(input: Tensor, target: Tensor, reduce_batch_first: bool = False, epsilon=1e-6):
    ...
    if input.dim() == 2 or reduce_batch_first:
        inter = torch.dot(input.reshape(-1), target.reshape(-1))  # 如果输入与目标形状一致,直接用torch.dot计算交集
        sets_sum = torch.sum(input) + torch.sum(target)  # 求并集是输入与目标各自和的和
        if sets_sum.item() == 0:
            sets_sum = 2 * inter
...
def multi_dice_score(net, dataloader, device, dice_ce_rate, num_classes=4):
    net.eval()
    num_val_batches = len(dataloader)
    dice_score, loss, all_lab, all_pre = 0, 0, [], []
    criterion = torch.nn.CrossEntropyLoss()
    with torch.no_grad():
        for (x, label) in tqdm(dataloader, desc='Validation round'):
    ...
    if num_classes == 5:
        return dice_score / num_val_batches, loss / num_val_batches, acc[num_classes], acc[num_classes + 1], acc[
            num_classes + 2], \
            acc[0], acc[1], acc[2], acc[3], acc[4]  # test_dice, test_loss, OA, AA, Kappa, acc0, acc1, acc2, acc3, acc4